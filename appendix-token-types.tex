...
\paragraph{Provenance growth.} The merge reason embeds $k$ complete tokens, each with its own history; repeated merging therefore grows provenance multiplicatively, bounded by $\Lambda$. The intended composition is with history compression (Sec.~\ref{sec:history-compression}): once active, sources SHOULD have their transfer histories compressed before merging, which makes each embedded source constant-size in its own transfer count. Note that each source must retain its terminal burn uncompressed, and that compression does not reduce $k$ or the depth of the sources' own ancestry.
...
\begin{itemize}
	...
	\item $\mathsf{RT} = \{\mathsf{ISSUER\_TAG}, 39044\}$ -- primary issuance by issuer authorization (Sec.~\ref{app:reason-issuer}); change-making by split (Sec.~\ref{sec:token-splitting}). The set MAY additionally include $\mathsf{MERGE\_TAG}$ to permit consolidation (Sec.~\ref{app:reason-merge}). A \emph{bridged} fungible asset is the variant with $\mathsf{RT} = \{39048, 39044\}$ and the bridge configuration $h_\mathsf{cfg}$ as issuance parameter (Sec.~\ref{sec:bridged-assets}).
...
\begin{itemize}
	...
	\item $\mathsf{RT} = \{\mathsf{SLOT\_TAG}\}$, optionally extended with 39044 and $\mathsf{MERGE\_TAG}$ --- both value-conserving, so the limit remains unconditional. No other issuance format may be admitted. History compression (Sec.~\ref{sec:history-compression}) is not an issuance format and never appears in $\mathsf{RT}$; it is available to capped types like any other and does not bear on the cap.
\end{itemize}
