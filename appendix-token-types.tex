...
Consequently, for a type whose admissible set satisfies $\mathsf{RT} \subseteq \{\mathsf{SLOT\_TAG}, 39044, \mathsf{MERGE\_TAG}\}$, at most $N$ geneses are ever created by direct issuance, and every other admitted format strictly conserves value (Secs.~\ref{sec:split-verification}, \ref{app:reason-merge}). The total value ever issued is therefore bounded by $\sum_{j} \Call{Value}{b_j}$, and the token count of a count-capped series by $N$. Admitting any non-conserving format (e.g.\ $\mathsf{ISSUER\_TAG}$) alongside $\mathsf{SLOT\_TAG}$ voids the bound; the admissible set is part of the frozen manifest, so a verifier sees at type-registration time whether the limit is unconditional.
...
Unlike a split, a merge needs no allocation tree: value conservation is direct arithmetic over the sources' verified payloads. Like the split reason it is in \emph{reference form} (Sec.~\ref{sec:token-evidence}) --- the reason names its sources by identifier and their evidence travels beside the token --- so a merged token's provenance is compressible and repeated merging does not compound (see \emph{Provenance growth} below).
...
The mint reason is a CBOR array under tag $\mathsf{MERGE\_TAG}$ containing exactly two items: the format version ($1$) and the source identifiers $\langle \mathsf{id}_1, \ldots, \mathsf{id}_k\rangle$ in strictly increasing order. Everything else --- the sources and their terminal burns --- is resolved from the evidence set (Sec.~\ref{sec:token-evidence}), each source as a complete token or as a token attestation. As in a split, the burns need no commitment in the reason: each source's pre-burn state fixes a state identifier, and at most one transaction is ever certified for it (Sec.~\ref{sec:request-validation}).
...
\begin{enumerate}
	...
	\item Mint the output with the reason naming the burned sources, and assemble the evidence set: for each source, the source itself in full or as an attestation, its certified burn transaction, and transitively the entries those name (Sec.~\ref{sec:token-evidence}).
\end{enumerate}
...
\begin{enumerate}
	...
	\item Require the identifiers $\mathsf{id}_i$ strictly increasing in $i$. For each: resolve $\mathsf{id}_i$ in the evidence set $\mathcal{E}$ (Sec.~\ref{sec:token-evidence}) --- a missing entry rejects --- either handing a complete token to $\mathsf{collect}$ for worklist verification (Sec.~\ref{sec:nested-verification}) or verifying a token attestation by $\Call{VerifyAttestation}{}$, which must not return $\bot$ (Sec.~\ref{sec:attestation}). Take $(\mathsf{ty}_i, \mathcal{A}_i, h_i, e_i)$ from the resolved evidence; require the resolved identifier to equal $\mathsf{id}_i$, the type to be byte-identical to $D_\mathsf{mint}.\mathsf{ty}$, and the network identifier to be $\mathcal{T}.\alpha$.
	\item Recompute $d$ from $\langle\mathsf{id}_1,\ldots,\mathsf{id}_k\rangle$ and the output mint fields other than $\mathsf{reason}$; form $b_\mathcal{M}$. For each source require $\mathcal{E}$ to supply a certified transfer out of $(\decpred(e_i), h_i)$, verified as an ordinary transfer (Sec.~\ref{sec:transfer-tx}), with auxiliary data exactly $b_\mathcal{M}$ and recipient predicate exactly $\predi_\mathsf{burn}(\mathsf{SHA\text{-}256}(b_\mathcal{M}))$; when a source was supplied as a complete token this is its final transfer.
...
\paragraph{Provenance growth.} With sources supplied in full, a merge carries $k$ complete tokens and repeated merging grows provenance multiplicatively, bounded by $\Lambda$. The intended composition is with compression (Sec.~\ref{sec:token-compression}): sources SHOULD be supplied as token attestations, prepared in advance by each source holder. Each source then costs one constant-size attestation irrespective of its own history and ancestry, and the merged token's own attestation --- prepared later --- folds all $k$ into a single proof. What survives is the issuance events: each independent lineage entering the merge contributes one genesis that every verifier checks in the clear (Sec.~\ref{sec:attestation}), so a merged token's cost is proportional to the number of distinct issuance events behind it rather than to its transaction history. Until that fold, each source presents its terminal burn in the clear, in $\mathcal{E}$; the fold verifies those burns as transfer steps in-circuit, after which they no longer travel with the merged token (Sec.~\ref{sec:compression-scope}).
...
\begin{itemize}
	...
	\item $\mathsf{RT} = \{\mathsf{ISSUER\_TAG}, 39044\}$ -- primary issuance by issuer authorization (Sec.~\ref{app:reason-issuer}); change-making by split (Sec.~\ref{sec:token-splitting}). The set MAY additionally include $\mathsf{MERGE\_TAG}$ to permit consolidation (Sec.~\ref{app:reason-merge}); a type intended for high circulation SHOULD, since provenance breadth rather than depth is what survives compression (Sec.~\ref{sec:compression-scope}). A \emph{bridged} fungible asset is the variant with $\mathsf{RT} = \{39048, 39044\}$ and the bridge configuration $h_\mathsf{cfg}$ as issuance parameter (Sec.~\ref{sec:bridged-assets}).
...
\begin{itemize}
	...
	\item $\mathsf{RT} = \{\mathsf{SLOT\_TAG}\}$, optionally extended with 39044 and $\mathsf{MERGE\_TAG}$ --- both value-conserving, so the limit remains unconditional. No other issuance format may be admitted. Compression (Sec.~\ref{sec:token-compression}) is not an issuance format and never appears in $\mathsf{RT}$; it is available to capped types like any other and does not bear on the cap. A capped type's slot geneses are exactly the opaque geneses of its holders' attestations, checked in the clear by every verifier under its own registry, so the cap is re-established on every verification exactly as it is for an uncompressed token. Capped types are also the case in which compression is not substitutable by issuer reissue (Sec.~\ref{sec:compression-scope}): an issuer able to authorize a fresh equivalent token would void the cap.
\end{itemize}
...
\begin{itemize}
	...
	\item \emph{Verification cost and compression}: pool verification cost grows with transition count, and each deposit/withdrawal embeds a burned token. Practical pools depend on compression of $\mathsf{ty}_\mathsf{P}$ tokens, and on restating the pool transition records in reference form so that they can be compressed at all (Sec.~\ref{sec:token-evidence}). Since the pool type also defines per-transfer checks, the generic relation $\mathfrak{R}_\mathsf{tok}$ (Sec.~\ref{sec:compression-relation}) does not cover it: a variant carrying the pool transition checks is required, and $\mathsf{ty}_\mathsf{P}$'s manifest must pin its verification key --- tracked in the roadmap.
...
Token compression: evidence set, attestations, $\mathfrak{R}_\mathsf{tok}$ (Sec.~\ref{sec:token-compression}) & draft & select proof-system profile; encode $\mathcal{K}$ and the evidence set; relation variant for per-transfer type checks; reference form for conversion and pool records \\
Root history accumulator (Sec.~\ref{sec:root-history}) & draft & review; encoding and serving discipline for $\pi^\mathsf{rh}$ \\
