...
Consequently, for a type whose admissible set satisfies $\mathsf{RT} \subseteq \{\mathsf{SLOT\_TAG}, 39044, \mathsf{SPLITREF\_TAG}, \mathsf{MERGE\_TAG}\}$, at most $N$ geneses are ever created by direct issuance, and every other admitted format strictly conserves value (Secs.~\ref{sec:split-verification}, \ref{app:reason-splitref}, \ref{app:reason-merge}). The total value ever issued is therefore bounded by $\sum_{j} \Call{Value}{b_j}$, and the token count of a count-capped series by $N$. Admitting any non-conserving format (e.g.\ $\mathsf{ISSUER\_TAG}$) alongside $\mathsf{SLOT\_TAG}$ voids the bound; the admissible set is part of the frozen manifest, so a verifier sees at type-registration time whether the limit is unconditional.
...
\begin{itemize}
	...
	\item $\mathsf{RT} = \{\mathsf{ISSUER\_TAG}, 39044\}$ -- primary issuance by issuer authorization (Sec.~\ref{app:reason-issuer}); change-making by split (Sec.~\ref{sec:token-splitting}). The set MAY additionally include $\mathsf{MERGE\_TAG}$ to permit consolidation (Sec.~\ref{app:reason-merge}) and $\mathsf{SPLITREF\_TAG}$ (Sec.~\ref{app:reason-splitref}) to make split-minted tokens' provenance compressible; a type intended for high circulation SHOULD admit the reference form and its wallets SHOULD prefer it. A \emph{bridged} fungible asset is the variant with $\mathsf{RT} = \{39048, 39044\}$ and the bridge configuration $h_\mathsf{cfg}$ as issuance parameter (Sec.~\ref{sec:bridged-assets}).
...
\begin{itemize}
	...
	\item $\mathsf{RT} = \{\mathsf{SLOT\_TAG}\}$, optionally extended with 39044, $\mathsf{SPLITREF\_TAG}$ and $\mathsf{MERGE\_TAG}$ --- all value-conserving, so the limit remains unconditional. No other issuance format may be admitted. Compression (Secs.~\ref{sec:history-compression}, \ref{sec:provenance-compression}) is not an issuance format and never appears in $\mathsf{RT}$; it is available to capped types like any other and does not bear on the cap. A capped type's slot geneses are the issuance roots of its holders' certificates, checked in the clear by every verifier, so the cap is re-established on every verification exactly as it is for an uncompressed token.
\end{itemize}
