...
\paragraph{Leaf.} Against the authenticated table, a leaf $L = (\mathsf{sid}, \txhash)$ is established by
\[
...
\begin{enumerate}
  \item $\Call{rsmt\_verify\_inclusion}{C^\mathsf{inc}, \mathcal{IR}.h, \mathsf{sid}, \txhash} = 1$ \quad (the leaf is in the shard's SMT);
...
With it, one recent seal authenticates the roots of all earlier rounds, so an arbitrarily long history --- and, in Sec.~\ref{sec:token-compression}, an arbitrarily deep provenance --- collapses to one signature verification. It also means the relying party never needs the trust base of an earlier epoch: authority flows from the one seal it verifies under its \emph{current} trust base, and an old seal from an abandoned fork cannot be substituted, because its root is not in the accumulator of the chain the relying party is on.
...
Because the anchored path runs through the round's own input record, the reference time is available: $\tau = \mathcal{IR}.t$ of the round in which the leaf was certified (Sec.~\ref{sec:time-extraction}), authenticated by the same check~5. A time-dependent predicate --- $\predi_\mathsf{tlock}$, $\predi_\mathsf{htlc}$ --- is therefore evaluated under an anchored proof exactly as under an ordinary inclusion proof, at the cost of one extra root history path, and no per-transition Unicity Certificate is needed.

This requires the leaf to be anchored at \emph{its own} round's root rather than at the anchor round's root: a recent root proves that the leaf exists, but its input record carries the recent round's reference time, not the leaf's. Both forms are admitted by check~5, and a prover uses the leaf's own round exactly when the transition's predicate reads $\tau$.
