...
Algorithm~\ref{alg:bridge-relation} defines the asset-specific relation $\mathfrak{R}_{\mathsf{br},\mathcal{C}}$: the relation holds iff every check passes for the configuration $\mathcal{C}$ pinned by the vault. A common program may realize many instances by taking $h_\mathsf{cfg}$ as public input, provided the vault requires $h_\mathsf{cfg}=H_\mathsf{cfg}(\mathcal{C})$ and pins the corresponding verification key. The witness $\mathsf{w}$ comprises an opening of $\mathcal{C}$, the trust base $\mathcal{T}$, the anchor seal $C^{\mathsf{r}*}$, the round table $\mathcal{R}$ with its batched proof $\Psi$, and for each burn $i$: the source token $\mathcal{L}_i$, its anchored inclusion proofs, the lock record(s) backing its genesis, and the accumulator non-membership witness $w_i$. $\Call{VerifyToken}{}$ (Sec.~\ref{sec:token-verification}) is invoked in \emph{anchored} mode: wherever it would verify a certified transaction's own Unicity Certificate, it instead verifies an anchored leaf against the round table $\mathcal{R}$ (Sec.~\ref{app:bridging-anchor}). Its reference-form split-reason checks (Sec.~\ref{sec:split-verification}) supply value conservation, with each source resolved from the witness. Reference time is available from the anchored path (Sec.~\ref{app:anchor-time}), so time-dependent predicates are admitted. Unlike $\mathfrak{R}_\mathsf{tok}$, this relation verifies the bridged genesis against $\mathcal{C}$ and the lock record. It exports the lock reference as a public obligation, whose existence the vault checks against its recorded digest (Sec.~\ref{app:bridging-in-security}). External lock finality was established at bridge-in and need not be proved again at return.

A deployment that accepts token attestations (Sec.~\ref{sec:token-compression}) may verify $\pi_\mathsf{tok}$ recursively instead of expanding the represented token history. It SHOULD prefix-reconcile the attestation anchor with $a^*$, which adds hashes and no historical seal verification. If the prefix witness is unavailable, the seal fallback of Sec.~\ref{sec:compression-relation} is possible because $\mathcal{T}$ is already a witness, but it adds one historical quorum verification per absorbed attestation and inherits the fallback's historical-quorum assumption. The current anchor seal is verified once by $\mathfrak{R}_{\mathsf{br},\mathcal{C}}$ because the vault has no separate off-circuit seal-verification step.

\begin{algorithm}[!htbp]
\caption{Batch Return Relation $\mathfrak{R}_{\mathsf{br},\mathcal{C}}(\mathsf{x},\mathsf{w})$}\label{alg:bridge-relation}
\begin{algorithmic}[0]
\Function{BridgeReturnRelation}{$\mathsf{x},\mathsf{w}$}
  ...
  \For{$i \gets 1 \ldots \mathsf{x}.B$}
    ...
    \State $(T_{\mathsf{b},i},u_{\mathsf{b},i},\tau_{\mathsf{b},i},\pi_{\mathsf{b},i}) \gets$ final certified transfer of $\mathcal{L}_i$
    ...
    \State ensure($\Call{VerifyAnchoredLeaf}{\mathcal{T}.\alpha, \mathcal{R}, (\mathsf{sid}_{\mathsf{b},i},H(\txhash_{\mathsf{b},i},\tau_{\mathsf{b},i})), \pi_{\mathsf{b},i}} = 1$)
