...
Because the anchored path runs through the round's own input record, the reference time is available as $\tau = \mathcal{IR}.t$ of the round in which the leaf was certified (Sec.~\ref{sec:time-extraction}), authenticated by check~4 above. A time-dependent predicate --- $\predi_\mathsf{tlock}$, $\predi_\mathsf{htlc}$ --- is therefore evaluated under an anchored proof exactly as under an ordinary inclusion proof, at the cost of one extra root history path, and no per-transition Unicity Certificate is needed.

This requires the leaf to be presented at \emph{its own} round's root rather than at a later round's root. A later root proves that the leaf exists, but its input record carries that later round's reference time, not the leaf's. Both forms are admitted by check~4, and a prover uses the leaf's own round exactly when the transition's predicate reads $\tau$. Presenting a leaf at its own round requires a root history path for a round that may be long past, so a prover doing so depends on a node retaining that range (Sec.~\ref{sec:root-history-pruning}); leaves whose predicates do not read $\tau$ carry no such dependency, because they can be re-presented against a recent round (Sec.~\ref{sec:leaf-representation}).
