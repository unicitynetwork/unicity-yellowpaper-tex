...
Note that $\mathcal{C}$ pins \emph{which} asset identifier is bridged but not how $\Call{Assets}{}$ decodes it (Sec.~\ref{sec:asset-structure}) --- that decoding is fixed by the token type $\mathsf{ty}$ itself and must be identical across every independent implementation that mints, backs, or returns the bridged token.
...
\Function{BridgeReturnRelation}{$\mathsf{x},\mathsf{w}$}
  ...
  \For{$i \gets 1 \ldots \mathsf{x}.B$}
    ...
    \State $\mathcal{R}_i \gets \Call{DecodeReason}{T_\mathsf{b}.D.\auxd'}$ such that $T_\mathsf{b}.\predi' = \predi_\mathsf{burn}(H(T_\mathsf{b}.D.\auxd'))$
