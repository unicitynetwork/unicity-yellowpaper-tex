...
Let $\mathsf{BURN\_DOMAIN}$ be the fixed byte string \texttt{unicity-burn-transition:v1}. Let the burn be the final certified transfer of the source token. It spends the state $(\predi_{n-1},\sthash_{n-1})$ and its transaction hash is $\txhash_\mathsf{b}=H(T_\mathsf{b}.D,\tau_{Q,\mathsf b})$. The certified Unicity leaf for that burn is keyed by
\[
...
\Function{BridgeReturnRelation}{$\mathsf{x},\mathsf{w}$}
  ...
  \For{$i \gets 1 \ldots \mathsf{x}.B$}
    ...
    \State $(T_{\mathsf{b},i},u_{\mathsf{b},i},\tau_{Q,\mathsf{b},i},\tau_{\mathsf{b},i},\pi_{\mathsf{b},i}) \gets$ final certified transfer of $\mathcal{L}_i$
    ...
    \State $\mathsf{sid}_{\mathsf{b},i} \gets H(\predi_{\mathsf{b},i},\;T_{\mathsf{b},i}.\sthash)$;\quad $\txhash_{\mathsf{b},i} \gets H(T_{\mathsf{b},i}.D,\tau_{Q,\mathsf{b},i})$
    \State ensure($\tau_{\mathsf{b},i}<\tau_{Q,\mathsf{b},i}$ and $\pi_{\mathsf{b},i}$ proves $(\mathsf{sid}_{\mathsf{b},i},H(\txhash_{\mathsf{b},i},\tau_{\mathsf{b},i}))$ under $h^*$)
