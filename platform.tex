...
For the SMT-based aggregation, the partition maintains a sparse Merkle tree mapping state identifiers to certified leaf values. The hash function $\mathsf{hf}$ is used for all cryptographic operations, and the encoding scheme $\mathsf{enc}$ specifies how data structures are serialized before hashing (see Sec.~\ref{sec:smt} for SMT structure).
...
A \emph{Unicity Service Request}\index{$Q$ (unicity service request)} is a request to the Unicity Service for state transition certification. Let $\mathbb{T}_\bot=\mathbb{T}\cup\{\bot\}$. A request is a tuple $Q = (\predi, \sthash, \txhash, \bar\tau_Q, u)$, where:
\begin{itemize}
	\item $\predi$ -- current owner's spending predicate (Sec.~\ref{sec:predicates})
	\item $\sthash \in \hashtype$ -- current state hash
	\item $\txhash \in \hashtype$ -- transaction hash
	\item $\bar\tau_Q \in \mathbb{T}_\bot$ -- an optional explicit, exclusive request timeout
	\item $u$ -- unlocking argument satisfying $\predi$
\end{itemize}

The transaction hash commits to $\bar\tau_Q$ in either case. The timeout occupies a fixed position in the transaction encoding, and $\bar\tau_Q=\bot$ is written there as the CBOR simple value \texttt{null} (Appendix~\ref{app:encodings}), so omitting a timeout requires no client clock. Let $\Delta>0$ be the service's default request lifetime and let $\tau_a$ be its latest consensus-derived reference time when the request is admitted. The \emph{effective timeout} is
\[
	\widehat\tau_Q =
	\begin{cases}
		\tau_a+\Delta, & \bar\tau_Q=\bot,\\
		\bar\tau_Q, & \bar\tau_Q\ne\bot.
	\end{cases}
\]
Thus omission of a timeout requires neither a local wall clock nor a trusted client clock. The value $\widehat\tau_Q$ is service metadata and does not alter $\txhash$.

For every request materialized in a round with reference time $\tau$, the certified value is
\[
	\lambda(Q,\tau)=H(\txhash,\tau).
\]
This relation is independent of whether the request timeout was explicit or assigned by the service.

The Unicity Service processes the request by:
\begin{enumerate}
	\item Checking the effective timeout: $\tau < \widehat\tau_Q$
	...
	\item If all checks pass: setting the SMT leaf $(\mathsf{sid}, \lambda(Q,\tau))$ in the tree
\end{enumerate}

The timeout constrains validation only. The authoritative check is made when the leaf is materialized, using $\tau=\mathcal{IR}.t$ for that round; certification and delivery may occur later. An earlier admission check may reject requests that are already expired, but cannot replace the materialization-time check.
...
Given a Unicity Service Request $Q = (\predi, \sthash, \txhash, \bar\tau_Q, u)$ and its assigned effective timeout $\widehat\tau_Q$, the aggregation layer node performs the following checks:
...
\begin{enumerate}
	...
	\item \textbf{Timeout check}: Verify
	\[
	\tau < \widehat\tau_Q .
	\]
...
\[
\text{Set SMT leaf: } (\mathsf{sid}, \lambda(Q,\tau))
\]
This updates the SMT by inserting the new leaf.

If either check fails, the request is rejected:
\begin{itemize}
	\item If the timeout check fails: the request is expired and must be rejected
...
\begin{itemize}
	...
	\end{itemize}
	\item \textbf{Value}: $v=\lambda(Q,\tau)=H(\txhash,\tau)$.
\end{itemize}

The SMT effectively maintains a mapping $\mathsf{sid} \mapsto H(\txhash,\tau)$ that proves a state transition from state identifier $\mathsf{sid}$ was authorized at the committed reference time. This prevents double-spending: once a state identifier is registered in the SMT, that state cannot be spent again.
...
\begin{itemize}
	...
	\item \textbf{Inclusion Certificate} $C^\mathsf{inc}$: authenticates that a leaf $(\mathsf{sid}, \lambda(Q,\tau))$ is present under a given SMT root.
...
\emph{Inclusion Proof}\index{$\pi^\mathsf{inc}$ (inclusion proof)}\index{inclusion proof} proves that a state identifier has been recorded in the SMT as a proof of the spend being unique, and that the resulting shard state was certified by the BFT Core. It is a tuple $\pi^\mathsf{inc} = (\mathsf{sid}, v, \tau, C^\mathsf{inc}, UC)$, where:
\begin{enumerate}
	\item $\mathsf{sid} \in \bytes{32}$ -- state identifier (SMT leaf key)
	\item $v \in \hashtype$ -- SMT leaf value, $v = H(\txhash,\tau)$ (Execution Layer, Sec.~\ref{sec:us-processing})
...
\textbf{Input}:
\begin{enumerate}
	\item $\pi^\mathsf{inc} = (\mathsf{sid}, v, \tau, C^\mathsf{inc}, UC)$ -- inclusion proof
...
\begin{enumerate}
	...
	\item $B = \langle (\mathsf{sid}_1, v_1), \ldots, (\mathsf{sid}_j, v_j) \rangle$ -- batch of inserted leaves
...
For the hash-based instantiation of the consistency proof in this model, a declared request batch $\widetilde B$ (the sorted list of $(\mathsf{sid}, h_\mathsf{tx})$ pairs) accompanies the opcode stream on the wire, since the BFT Core does not otherwise possess it; the check $|\widetilde B| = \mathsf{CR}.\ell_B$ cross-links it to the certification request. Supplying $\widetilde B$ from the untrusted aggregator is safe in this model: append-only consistency holds for the leaf batch derived from whatever request batch is declared, and batch contents beyond that are covered by the scoping argument above.

\paragraph{Derived leaf values.}\label{sec:derived-leaf-values}
Let
\[
	\widetilde B = \bigl\langle (\mathsf{sid}_i,h_{\mathsf{tx},i}) \bigr\rangle_{i=1}^{j}
\]
be the declared request batch. Before verifying the proof, the BFT Core derives the concrete leaf batch
\[
	B^{*} \;=\; \bigl\langle\, (\mathsf{sid}_i,\; H(h_{\mathsf{tx},i},\tau)) \,\bigr\rangle_{i=1}^{j},
	\qquad \tau = \mathsf{CR}.\mathcal{IR}.t ,
\]
and verifies $\mathsf{VerifyConsistencyProof}(\pi_{CP}, B^{*}, h', h)$. It already holds the required inputs: $\widetilde B$ arrives with the opcode stream, and $\tau$ is the value it checks against the previous seal (Sec.~\ref{certification-request}, Execution Layer Sec.~\ref{sec:time-extraction}). A shard that built any leaf under another reference time produces a root the Core does not reproduce.

Deriving rather than accepting a supplied leaf value makes a wrong $\tau$ unrepresentable. Were $\widetilde B$ to carry the leaf value directly, it would also have to carry the inputs needed to check the derivation. This does not extend the Core into semantic validation: it evaluates no predicate and parses no transaction. It applies a fixed derivation over fields already present in the certification request.

The ZK-compressed instantiation (Appendix~\ref{app:zk-consistency-proof}) keeps $\widetilde B$ private and exposes the two roots and $\tau$, deriving every leaf value inside the circuit so that both instantiations enforce the same relation.
...
\item $\mathsf{CR}.\mathcal{IR}.t = \mathcal{SI}[\beta, \sigma].UC_-.C^\mathsf{r}.t$ -- time reference is equal to the time field of the previous unicity seal
	\item $\mathcal{CD}[\beta].\gamma_{CP}(\mathsf{CR}.\mathcal{IR}, \mathsf{CR}.\pi_{CP})$ -- consistency proof verification (Sec.~\ref{sec:consistency-proof}) succeeds: derive $B^{*}$ from $\widetilde B$ and the reference time $\mathsf{CR}.\mathcal{IR}.t$ checked above (Sec.~\ref{sec:derived-leaf-values}), then verify $\mathsf{VerifyConsistencyProof}(\mathsf{CR}.\pi_{CP}, B^{*}, \mathsf{CR}.\mathcal{IR}.h', \mathsf{CR}.\mathcal{IR}.h) = \TRUE$ and $|\widetilde B| = \mathsf{CR}.\ell_B$ (batch size matches).
\end{enumerate}
