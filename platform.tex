...
See Fig.~\ref{fi:inc-proof} for an Inclusion Proof and Fig.~\ref{fi:uc} for the Unicity Certificate. The certificate $C^\mathsf{inc}$ authenticates the terminal SMT leaf $L=(\mathsf{sid},\txhash)$ against root $h$ in the Input Record. Despite its encoded type name, \texttt{InclusionCertificate}, the same format also proves non-inclusion when its key-directed path terminates at a different leaf. The Unicity Certificate then chains through the shard tree, unicity tree, and unicity seal to the global root of trust: Unicity Trust Base $\mathcal{T}$.
...
\index{$C^\mathsf{inc}$ (inclusion certificate)}\index{Inclusion Certificate}\index{Non-Inclusion Certificate}
...
\begin{itemize}
	...
	\item \textbf{Certificate} $C^\mathsf{inc}$: authenticates the leaf reached by key-directed descent for a target state identifier. A terminal key equal to the target proves inclusion (and authenticates its value); a different terminal key proves non-inclusion. The encoded type \texttt{InclusionCertificate} covers both cases.
...
\subsection{Inclusion and Non-Inclusion Proofs}

The encoded data structure is named \texttt{InclusionProof}\index{$\pi^\mathsf{inc}$ (inclusion proof)}\index{inclusion proof}\index{non-inclusion proof}\index{exclusion proof} and is the tuple
\[
	\pi^\mathsf{inc}=(C^\mathsf{inc},UC),
\]
where $C^\mathsf{inc}$ has encoded type \texttt{InclusionCertificate} (Appendix~\ref{sec:rsmt-inclusion-cert}) and $UC$ is the Unicity Certificate authenticating its root. These type names denote the shared encodings, not the relation asserted: both structures also cover non-inclusion. The target state identifier $\mathsf{sid}$ is supplied by the verifier and is not duplicated in the proof. Define $\pi^\mathsf{inc}.\mathsf{terminal}=\bot$ and $\pi^\mathsf{inc}.\mathsf{terminalKey}=\bot$ for the empty certificate; for a non-empty certificate ending at $(k',v')$, they are $(k',v')$ and $k'$, respectively. In an inclusion context, $\pi^\mathsf{inc}.\txhash$ denotes the terminal value $v'$.

An instance is an \emph{inclusion proof} for $(\mathsf{sid},\txhash)$ when its authenticated terminal equals $(\mathsf{sid},\txhash)$. It is a \emph{non-inclusion proof} for $\mathsf{sid}$ when its terminal key differs from $\mathsf{sid}$; this includes the empty-certificate case because $\bot\ne\mathsf{sid}$.

\subsubsection{Shared authentication: $\mathsf{AuthenticateSMTProof}$}\label{AuthenticateSMTProof}\index{functions!AuthenticateSMTProof}

$\mathsf{AuthenticateSMTProof}$ authenticates the certificate path, its certified root, and the shard to which the target key routes. It does not by itself select inclusion or non-inclusion; the caller MUST apply one of the terminal checks below.

\textbf{Input}:
\begin{enumerate}
	\item $\pi^\mathsf{inc}=(C^\mathsf{inc},UC)$ -- \texttt{InclusionProof} encoding (also used for non-inclusion)
	\item $\mathsf{sid}\in\bytes{32}$ -- target state identifier
...
\begin{algorithmic}[0]
	...
	\State ensure($f_{\mathcal{CD}_\beta.\mathcal{SH}}(\mathsf{sid}) = \sigma$) \Comment{Target is certified by the right shard}
	\If{$\neg\Call{rsmt\_verify\_certificate}{C^\mathsf{inc},UC.IR.h,\mathsf{sid}}$} \Return \FALSE \EndIf
	\State \Return $\mathsf{VerifyUnicityCert}(UC,\mathcal{T})$
\end{algorithmic}

\subsubsection{Inclusion relation: $\mathsf{VerifyInclusionProof}$}\label{VerifyInclusionProof}\index{functions!VerifyInclusionProof}

\[
\begin{split}
\mathsf{VerifyInclusionProof}(\pi^\mathsf{inc},\mathsf{sid},\txhash,\mathcal{CD}_\beta,\mathcal{T})
\;:=\;&\mathsf{AuthenticateSMTProof}(\pi^\mathsf{inc},\mathsf{sid},\mathcal{CD}_\beta,\mathcal{T})\\
&{}\land \pi^\mathsf{inc}.\mathsf{terminal}=(\mathsf{sid},\txhash).
\end{split}
\]
...
\subsubsection{Non-inclusion relation: $\mathsf{VerifyNonInclusionProof}$}\label{VerifyNonInclusionProof}\index{functions!VerifyNonInclusionProof}

\[
\begin{split}
\mathsf{VerifyNonInclusionProof}(\pi^\mathsf{inc},\mathsf{sid},\mathcal{CD}_\beta,\mathcal{T})
\;:=\;&\mathsf{AuthenticateSMTProof}(\pi^\mathsf{inc},\mathsf{sid},\mathcal{CD}_\beta,\mathcal{T})\\
&{}\land \pi^\mathsf{inc}.\mathsf{terminalKey}\ne\mathsf{sid}.
\end{split}
\]
The explicit terminal relation is security-critical: accepting only $\mathsf{AuthenticateSMTProof}$ would authenticate a terminal leaf but would not say whether the target key is present.
