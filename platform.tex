...
The identifier $\mathsf{st} = 2$ was used by a retired partition type and remains reserved; it must not be reassigned.
...
\begin{itemize}
	...
	\item \textbf{Coherent placement}: every leaf inserted in the round is placed at the position determined by its own key; together with the above, $M_1$ extends $M_0$ by exactly the bindings of $B$, whose keys must be fresh---no accepting proof exists for a batch that re-records an already-present key.
\end{itemize}
...
\section{State of the BFT Core}\label{sec:bft-core-state}

\emph{State of the BFT Core}\index{BFT Core!BFT Core state} is a tuple $(\alpha, n, e, r_-, \mathcal{T}, \mathcal{C}, \mathcal{CD}, \mathcal{SI}, \mathcal{G})$, where:
\begin{enumerate}
	...
	\item $\mathcal{G}$ -- staking registry (Sec.~\ref{sec:staking-registry})
\end{enumerate}
