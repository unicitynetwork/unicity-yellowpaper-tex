...
The accumulator is maintained by the BFT Core, retaining only the cumulative state (frontier) and exposing no public interface. Root history proofs are served by aggregators (as an optional service) and archive nodes. The root history is \emph{network-global}, verifiable public data and not shard-specific.

\subsubsection{Seal Distribution}\label{sec:seal-distribution}

To build the root history a node needs $r_n$ for \emph{every} round $n$, without gaps. A Certification Response (Sec.~\ref{certification-response}) delivers a seal only to shards whose input changed in that round.

The BFT Core MUST therefore publish each round's Unicity Seal to the aggregation layer, in every round, independently of whether any shard's input changed.

Aggregators MAY obtain seals from each other. A seal is self-authenticating based on Unicity Trust Base. A node that recovers the state can use whichever peer answers first.

\subsubsection{Ingestion}

On receiving the seal of round $n$, a node:
\begin{enumerate}
	\item verifies $C^\mathsf{r}.\alpha = \mathcal{T}.\alpha$ and $\mathsf{VerifyUnicitySeal}(C^\mathsf{r}.r, C^\mathsf{r}, \mathcal{T}) = 1$;
	\item requires $n$ to be exactly one past its current $N$, and requires the seal's accumulator to follow from its own frontier: $\Call{plain\_tree\_root}{P'} = C^\mathsf{r}.a_r$ where $(P', N') = \Call{AccAppend}{P, N, n, C^\mathsf{r}.r}$;
	\item appends $C^\mathsf{r}.r$ to its root log and adopts $(P', N')$.
\end{enumerate}

On failure the node recovers missing/corrupted state.

\subsubsection{Bootstrapping}

A new node recovers the root log as a batch from any peer and validates it in one pass: recompute the accumulator over $\langle r_0, \ldots, r_{N-1}\rangle$; the result must equal $a_r$ of a seal it has verified for round $N-1$. A node may validate against any \emph{later} seal, by additionally checking prefix consistency.

% \subsubsection{Retained State}

% \begin{itemize}
% 	\item the \emph{root log}: $r_n$ for every round, 32 bytes per round, append-only. At one round per second this is approximately 1\,GB per year;
% 	\item the frontier $(P, N)$, $O(\log N)$ hashes, derivable from the log;
% 	\item optionally, cached MMR interior nodes to avoid recomputing paths --- a further 32 bytes per round at most, and discardable at any time.
% \end{itemize}

% A node that serves root history MUST retain the log from round $0$: a path for even a recent round includes sibling material spanning older ranges, so a suffix is not sufficient.
...
\begin{description}
	\item[Ingestion] BFT Core $\rightarrow$ aggregators, once per round, as above. Continuous and independent of any client.
	\item[Replication] p2p batched protocol.
	\item[Proofs] A holder compressing a token obtains the rounds $n_1 < \cdots < n_d$ from the inclusion proofs of the token. It obtains an anchor by $\Call{GetSeal}{}$ or reuses an existing recent seal, and requests $\Call{GetRootHistoryBatch}{\langle n_1, \ldots, n_d\rangle, N^*}$. Folding of an existing attestation additionally requires one $\Call{GetRootHistoryPrefix}{}$.
	\item[Verification] Tokens are self-contained, thus, no query.
\end{description}

The serving node is trustless and can be replicated without coordination, cached, placed behind ordinary content-distribution infrastructure, or run privately by a semi-offline 3rd party.
