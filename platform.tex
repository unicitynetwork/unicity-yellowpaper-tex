...
The aggregation layer maintains a \emph{Radix Sparse Merkle Tree} (RSMT), a leaf-anchored, path-compressed binary Merkle tree over a 256-bit key space. The RSMT maps state identifiers to transaction hashes and provides compact inclusion and non-inclusion certificates. Refer to Appendix~\ref{app:rsmt} for details.
...
An \emph{Inclusion Certificate} $C^\mathsf{inc}$ for a leaf with key $\mathsf{sid}$ is a pair $C^\mathsf{inc} = (\mathit{bitmap}, \mathit{siblings})$ where:
\begin{itemize}
	\item $\mathit{bitmap} \in \bytes{32}$ -- a 256-bit vector; bit $d$ is set iff a sibling hash is required at depth $d$ on the path from the leaf to the root
	\item $\mathit{siblings} = \langle s_1, \ldots, s_n \rangle \in \hashtype^n$ where $n = \mathsf{bitcount}(\mathit{bitmap})$ -- sibling hashes ordered leaf-to-root
\end{itemize}

The verification function $\textsc{rsmt\_verify\_inclusion}$ is provided in Appendix~\ref{sec:rsmt-verify-inclusion}.
