...
A \emph{Unicity Service Request}\index{$Q$ (unicity service request)} is a request to the Unicity Service for state transition certification. It is a tuple $Q = (\predi, \sthash, \txhash, \tau_Q, u)$, where:
\begin{itemize}
	\item $\predi$ -- current owner's spending predicate (Sec.~\ref{sec:predicates})
	\item $\sthash \in \hashtype$ -- current state hash
	\item $\txhash \in \hashtype$ -- transaction hash, committing to the transaction data and $\tau_Q$
	\item $\tau_Q \in \mathbb{T}$ -- exclusive request timeout
	\item $u$ -- unlocking argument satisfying $\predi$
\end{itemize}

The Unicity Service processes the request by:
\begin{enumerate}
	\item Checking the request timeout: $\tau < \tau_Q$
	...
	\item If all checks pass: setting the SMT leaf $(\mathsf{sid}, H(\txhash,\tau))$ in the tree
\end{enumerate}

The timeout constrains validation only: a request or certified transaction is valid only if $\tau=\mathcal{IR}.t<\tau_Q$; certification and delivery may occur later.
...
Given a Unicity Service Request $Q = (\predi, \sthash, \txhash, \tau_Q, u)$, the aggregation layer node performs the following checks:
...
\[
\text{Set SMT leaf: } (\mathsf{sid}, H(\txhash,\tau))
\]
...
See Fig.~\ref{fi:inc-proof} for the Inclusion Proof structure and Fig.~\ref{fi:uc} for the Unicity Certificate. The Inclusion Proof contains an SMT leaf $L = (\mathsf{sid}, v)$ where $\mathsf{sid} = H(\predi, \sthash)$ and $v=H(\txhash,\tau)$. The Inclusion Certificate $C^\mathsf{inc}$ connects the leaf to the SMT root hash $h$ in the Input Record. The Unicity Certificate then chains through the shard tree, unicity tree, and unicity seal to the global root of trust: Unicity Trust Base $\mathcal{T}$. A Non-Inclusion Proof has the same shape, except that its certificate carries the leaf its key-directed path actually reaches instead of the queried one.
...
\begin{itemize}
	...
	\end{itemize}
	\item \textbf{Value}: $v=H(\txhash,\tau)$, where $\txhash$ commits to the transaction data and request timeout and $\tau$ is the request-validation reference time.
\end{itemize}

The SMT effectively maintains a mapping $\mathsf{sid} \mapsto H(\txhash,\tau)$ that proves a state transition from state identifier $\mathsf{sid}$ was authorized via transaction with hash $\txhash$ at reference time $\tau$. This prevents double-spending: once a state identifier is registered in the SMT, that state cannot be spent again.
...
\begin{enumerate}
	...
	\item $v \in \hashtype$ -- SMT leaf value, $v = H(\txhash, \tau)$ for the timeout-bound transaction hash and the request-validation reference time (Execution Layer, Sec.~\ref{sec:us-processing})
