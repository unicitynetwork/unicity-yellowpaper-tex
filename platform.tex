...
\begin{nidblock}
\paragraph{Network identifier as oracle binding.}\label{sec:networks-oracle}
The network identifier $\alpha$ identifies a specific \emph{Unicity oracle} -- the service authoritative for producing single-spend (unicity) proofs for transactions of states issued under that network. Two distinct networks $\alpha_1 \ne \alpha_2$ are independent oracles: each maintains its own spent-state registry and could, in isolation, certify conflicting spends of the same logical state. To eliminate this attack surface, every state $(\predi,\sthash)$ MUST be bound to a single network $\alpha$, and the unicity proof for spending that state MUST come from that very network. Inclusion proofs whose Unicity Certificate carries any other network identifier MUST be rejected at verification time, even if they would otherwise verify against a valid trust base. The binding mechanism is the predicate itself: $\alpha$ is a parameter of the predicate (Sec.~\ref{sec:predicate-framework}), and it is part of the canonical encoding $\encpred(\predi)$ that defines the state identifier $\mathsf{sid} = H(\predi,\sthash)$.

\paragraph{Realms.}\label{sec:realms}
A token state transition history may legitimately reference different network identifiers across consecutive states (e.g.\ ownership of a fungible token may move between two cooperating mainnet networks). To make such histories trust-equivalent for every relying party without ad-hoc trust decisions, we define a \emph{realm}\index{realm} as a publicly fixed set of network identifiers $R \subseteq \mathbb{A}$ whose oracles are mutually accepted within the realm. The realm membership is a global convention; SDKs and verifying clients MUST hardcode the realm-to-network-id mapping and MUST NOT accept user-supplied or dynamically negotiated realm definitions.

We define at least three realms:
\begin{description}
  \item[$\mathsf{MAINNET}$] -- all production-grade Unicity networks providing unicity proofs for transactions of tokens carrying real-value assets.
  \item[$\mathsf{TESTNET}$] -- networks providing unicity proofs for transactions of tokens that do not carry real-value assets (test, staging, demonstration).
  \item[$\mathsf{PRIVATE}$] -- networks providing unicity proofs for transactions of tokens issued for custom / consortium projects.
\end{description}

The realms are pairwise disjoint: $\mathsf{MAINNET}\cap\mathsf{TESTNET}=\mathsf{MAINNET}\cap\mathsf{PRIVATE}=\mathsf{TESTNET}\cap\mathsf{PRIVATE}=\emptyset$.

A token is bound to a single realm $R$ at genesis (Sec.~\ref{sec:mint-tx}). Every state in its transition history -- from the mint transaction up to and including any burn -- MUST reference a network identifier $\alpha \in R$. A token whose history contains two states referencing network identifiers from two different realms is invalid and MUST be rejected by every verifier, even if all its individual unicity proofs verify in isolation. Within a single realm, the per-state network identifier MAY change from one state to the next, allowing tokens to move between mutually-trusted oracles of the same realm without violating the realm-binding invariant.
\end{nidblock}
...
\textbf{PoW FP} ($\mathsf{st} = 2$) -- Finality Gadget partition for the PoW chain (Sec.~\ref{sec:fg-partition})
...
A \emph{Consistency Proof} (also called \emph{non-deletion proof}) is a cryptographic construction that validates one round of operation of the append-only SMT maintained by aggregation layer nodes. It proves that the state transition from previous SMT root hash $h'$ to current SMT root hash $h$ was performed correctly: only new leaves were added, and no pre-existing leaves were removed or modified. For a round with inserted batch
\[
...
The intended proof system properties are as follows. Specified algorithms enforce Items~1--4 below. Item~5 is a stronger structural property that is intentionally skipped.
\begin{enumerate}
	\item \textbf{Completeness}: If an honest aggregation layer node starts from the tree committed by $h'$ and inserts exactly the batch $B$, then it can produce $\pi_{CP}$ such that $\mathsf{VerifyConsistencyProof}(\pi_{CP}, B, h', h) = \TRUE$.
	\item \textbf{Soundness of prior-state preservation}: Let $M_0, M_1 \colon \bytes{32} \to \hashtype\opt$ denote the partial maps committed by $h'$ and $h$, respectively. If $\mathsf{VerifyConsistencyProof}(\pi_{CP}, B, h', h) = \TRUE$, then
	\[
	...
	\item \textbf{Batch incorporation}: If $\mathsf{VerifyConsistencyProof}(\pi_{CP}, B, h', h) = \TRUE$, then
	\[
		\forall\, i \in \{1, \ldots, j\}\colon\; M_1(\mathsf{sid}_i) = h_{\mathsf{tx},i}.
	\]
	Thus every declared inserted leaf appears in the post-state under the key determined by its state identifier.
	\item \textbf{No phantom insertions}: If $\mathsf{VerifyConsistencyProof}(\pi_{CP}, B, h', h) = \TRUE$, then
	\[
		\begin{aligned}
			\forall\, \mathsf{sid} \in \bytes{32}\colon\;& M_1(\mathsf{sid}) \ne \bot \land M_0(\mathsf{sid}) = \bot \\
			&\Rightarrow \exists\, i \in \{1, \ldots, j\}\colon\; (\mathsf{sid}, M_1(\mathsf{sid})) = (\mathsf{sid}_i, h_{\mathsf{tx},i}).
		\end{aligned}
	\]
	Therefore every new post-state leaf is justified by the declared batch.
	\item \textbf{Canonical inserts} (stronger structural property): Let $\mathsf{root}_{\mathsf{canon}}(M)$ denote the root hash of the unique well-formed radix sparse Merkle tree induced by a finite map $M$. The strongest desirable statement is
	\[
		h = \mathsf{root}_{\mathsf{canon}}(M_0 \cup B).
	\]
	Equivalently, the post-state topology is uniquely determined by the committed keys and every leaf in the domain of $M_0 \cup B$ admits a valid inclusion certificate against $h$.
\end{enumerate}

Equivalently, if $B$ is interpreted as a finite partial map with pairwise distinct keys disjoint from the domain of $M_0$, then acceptance of $\pi_{CP}$ implies
\[
	M_1 = M_0 \cup B.
\]

The specification deliberately omits Item~5 for efficiency reasons. In the worst case, a Byzantine aggregation layer node may obtain certification for a non-canonical post-state root that still preserves all previously committed leaves and incorporates the declared batch, yet leaves some committed leaves without valid inclusion certificates. Thus the loss is in structural canonicity and proof availability, not in prior-state preservation or batch incorporation.
...
\begin{enumerate}
	...
	\item $\dots$ -- other necessary data: fees, etc.
\end{enumerate}
