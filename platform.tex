...
The proof system must enforce the following property:
...
This property is required to avoid potential double-spending with the help of malicious operators.

The security model where an untrusted Aggregator machine generates (non-)inclusion proofs, anchored by committed root hashes, allows us to avoid stronger consistency guarantees like \emph{completeness} (free service is not a big issue), \emph{batch incorporation} (failure to inserting some leaves results in denial of service at worst), \emph{batch binding} (inclusion proofs do it better), \emph{no phantom insertions} (free service), \emph{canonical inserts} (denial of service at worst).
