...
A mint transaction has the common data
...
The payload is $D_\mathsf{mint}$ followed by the request timeout $\tau_Q$ at index~6, encoded as an unsigned integer, or as the CBOR simple value \texttt{null} when $\tau_Q=\bot$ (Appendix~\ref{app:encodings}). A versioned tagged envelope carries the payload.
...
A transfer transaction (spending a mint or prior transaction output) has five common elements:
...
The payload contains these common elements followed by the request timeout $\tau_Q$, encoded as an unsigned integer, or as the CBOR simple value \texttt{null} when $\tau_Q=\bot$ (Appendix~\ref{app:encodings}). A versioned tagged envelope carries the payload.


\subsection{Certified Transaction}

A certified transaction (mint or transfer) is encoded as a three-element CBOR array:

\begin{longtable}{clp{8cm}}
\textbf{Index} & \textbf{Type} & \textbf{Description} \\
\hline
0 & CBOR array & The transaction \\
1 & unsigned integer & Reference time $\tau$ of the round in which the leaf was created \\
2 & CBOR structure & Inclusion proof $\pi$ \\
\end{longtable}

The inclusion proof also carries the validation reference time, and a decoder requires it to agree with the value in the certified transaction. The request timeout $\tau_Q$ is inside the transaction at index~0.
...
For mint transactions:
\[
\txhash = \mathsf{SHA\text{-}256}(\mathsf{CBOR}(\alpha, \predi', \mathsf{salt}, \mathsf{ty}, \mathsf{reason}, \auxd', \tau_Q))
\]

For transactions:
\[
\txhash = \mathsf{SHA\text{-}256}(\mathsf{CBOR}(\predi', x, \auxd', \tau_Q))
\]

Here $\mathsf{CBOR}(\ldots)$ denotes deterministic CBOR array encoding. The timeout is always present as the last element and is therefore always committed by the transaction hash; $\tau_Q=\bot$ is encoded as \texttt{null} and requires no clock.
...
\begin{itemize}
	...
	\item \textbf{Signed message}: the predicate message hash $m = \mathsf{SHA\text{-}256}(\mathsf{CBOR}(\sthash, \txhash))$, where $\txhash$ commits to the exclusive request timeout, as defined in Sec.~\ref{sec:predicate-framework}.
\end{itemize}
