...
A certified transaction (mint or transfer) is encoded as a CBOR array with four elements:

\begin{longtable}{clp{8cm}}
\textbf{Index} & \textbf{Type} & \textbf{Description} \\
\hline
0 & CBOR array & The transaction \\
1 & unsigned integer & Exclusive request timeout $\tau_Q$ \\
2 & unsigned integer & Validation reference time $\tau$ \\
3 & CBOR structure & Inclusion proof $\pi$ \\
\end{longtable}
...
where $\tau_Q$ is carried by the certified transaction and $\mathsf{CBOR}(\ldots)$ denotes deterministic CBOR array encoding.
...
\begin{itemize}
	...
	\item \textbf{Signed message}: the predicate message hash $m = \mathsf{SHA\text{-}256}(\mathsf{CBOR}(\sthash, \txhash))$, where $\txhash$ commits to the exclusive request timeout $\tau_Q$, as defined in Sec.~\ref{sec:predicate-framework}.
\end{itemize}
