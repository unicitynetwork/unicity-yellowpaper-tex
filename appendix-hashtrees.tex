...
Let $s$ denote the next key-bit position examined during a top-down traversal.

\begin{itemize}
	\item \textbf{Empty tree}: The empty tree has root $\bot$.
	\item \textbf{Leaf node}: A leaf stores a pair $(k, v)$ with $k \in \bytes{32}$ and $v \in \bytestr$. The full key $k$ anchors the leaf position in the tree.
	\item \textbf{Internal node}: An internal node stores a tuple $(q, d, c_0, c_1)$, where $q \in \bits{d-s}$ is the maximal common path segment shared by all descendant keys from bit positions $s, \ldots, d-1$, $d \in \{0, \ldots, 255\}$ is the bifurcation depth, and $c_0$ and $c_1$ are the left and right child subtrees. A search key $k$ may descend through the node if and only if its bits $k[s], \ldots, k[d-1]$ equal $q$; the next bit $k[d]$ then selects child $c_0$ for $0$ and child $c_1$ for $1$.
\end{itemize}

The path segment $q$ is stored for routing but is not part of the Merkle hash. The commitment is determined entirely by the leaf contents, the child hashes and the absolute bifurcation depth.
...
The internal-node hash does not include the compressed path segment $q$. Every leaf hash already commits to the full key, and every internal-node hash commits to the absolute branching depth $d$.
...
\begin{algorithmic}
	\Function{rsmt\_batch\_insert\_np}{$T, B$}
		...
		\If{$B' = \langle \rangle$}
			\State \Return
		\EndIf
		\State $T.\mathsf{root} \gets \Call{rsmt\_merge\_batch}{T.\mathsf{root}, B'; 0}$
	\EndFunction
\end{algorithmic}


The helper procedure $\textsc{rsmt\_merge\_batch}(r, S; s)$ receives a current subtree root $r$, a contiguous slice $S$ of the traversal-sorted batch, and the next bit position $s$. The following pseudocode omits low-level bit manipulations.

\begin{algorithmic}
	\Function{rsmt\_merge\_batch}{$r, S; s$}
		\If{$S = \langle \rangle$}
			\State \Return $r$
		\EndIf
		\If{$r = \bot$}
			\State \Return \Call{rsmt\_build\_subtree}{S; s}
		\EndIf
		\If{$r$ is a leaf $(k_\ell, v_\ell)$}
			...
			\If{$\len{U} = 1$}
				\State \Return $r$
			\EndIf
			\State \Return \Call{rsmt\_build\_subtree}{U; s}
		\EndIf
		\State parse $r$ as internal node $(q, d, r_0, r_1)$
		\If{the first or last key of $S$ diverges from prefix $q$ before depth $d$}
			\State let $t < d$ be the first divergent depth
			\State partition $S$ into $S_0$ and $S_1$ according to bit $t$
			\State let $b \in \{0,1\}$ be the branch of the old subtree at depth $t$, as determined by $q$
			\If{$b = 0$}
				\State $r_0' \gets \Call{rsmt\_merge\_batch}{r, S_0; t}$
				\State $r_1' \gets \Call{rsmt\_merge\_batch}{\bot, S_1; t}$
			\Else
				\State $r_0' \gets \Call{rsmt\_merge\_batch}{\bot, S_0; t}$
				\State $r_1' \gets \Call{rsmt\_merge\_batch}{r, S_1; t}$
			\EndIf
			\State \Return a new parent node at depth $t$ with children $r_0'$ and $r_1'$
		\EndIf
		\State partition $S$ into $S_0$ and $S_1$ according to bit $d$
		\State $r_0' \gets \Call{rsmt\_merge\_batch}{r_0, S_0; d}$
		\State $r_1' \gets \Call{rsmt\_merge\_batch}{r_1, S_1; d}$
		\State \Return the node $(q, d, r_0', r_1')$
	\EndFunction
\end{algorithmic}
...
The helper procedure $\textsc{rsmt\_build\_subtree}(S; s)$ is applied only to a non-empty traversal-sorted slice:

\begin{enumerate}
	\item If $\len{S} = 1$, create a leaf for the single pair in $S$ and return it.
	\item Otherwise compute the first bit position $d \ge s$ at which the first and the last key of $S$ differ. This determines the unique bifurcation depth of the subtree root.
	\item Partition $S$ into $S_0$ and $S_1$ according to bit $d$, recursively build the two child subtrees, and return the internal node whose compressed path is the common bit string from positions $s, \ldots, d-1$.
\end{enumerate}
...
\begin{algorithmic}
	\Function{rsmt\_split\_node}{$r, s, b$}
		\If{$r$ is a leaf $(k, v)$}
			...
			\EndIf
		\EndIf
		\State parse $r$ as internal node $(q, d, c_0, c_1)$
		\If{$d = s$}
			\State \Return $c_b$ \Comment{selector bit resolves the split}
		\Else \Comment{$d > s$: all descendant keys agree on bit $s$}
			\State let $b^* \in \{0,1\}$ be bit $s$ of any descendant key (equivalently, the bit of $q$ at offset $s$)
			\If{$b^* = b$}
				\State \Return $r$
			\Else
				\State \Return $\bot$
...
\textbf{Properties}.
\begin{itemize}
	\item \emph{No rehashing}: Because internal-node hashes commit to the absolute bifurcation depth $d$ (Sec.~\ref{sec:rsmt-node-hash}) and leaf hashes commit to the full key (Sec.~\ref{sec:rsmt-leaf-hash}), every retained node keeps its pre-split hash. The post-split root hash equals the hash of whichever existing node the procedure returns.
	\item \emph{Inclusion Proof change}: An inclusion certificate (Sec.~\ref{sec:rsmt-inclusion-cert}) that was valid against the pre-split root is now different, the sibling entry at depth $s$, if any, from both the bitmap and the sibling list is dropped. This is compensated by the Unicity Certificates being larger, due to extra entry in Shard Tree.
	\item The procedure may be executed as many times as necessary to obtain necessary number of shards.
\end{itemize}
...
The current implementation proves preservation of committed pre-state subtrees and incorporation of the declared batch into the post-state hash. It does \emph{not} additionally prove that the resulting post-state is the unique canonical radix sparse Merkle tree induced by the final key set. This stronger structural property is omitted for efficiency reasons: enforcing it would require additional witness data, larger inclusion proofs and more expensive verification.
