...
The Unicity Service evaluates the predicate at the current system time $\tau$ when processing a certification request. Verifiers of certified transactions evaluate the predicate at the system time $\mathsf{time}(\pi)$ extracted from the inclusion proof.\index{$\tau$}

\index{time extraction}
Given an inclusion proof $\pi^\mathsf{inc} = (\mathsf{sid}, \txhash, C^\mathsf{inc}, UC)$, the \emph{system time} is extracted as
\[
	\mathsf{time}(\pi^\mathsf{inc}) \;=\; UC.C^\mathsf{r}.t_r
\]
where $t_r$ is the round creation time from the Unicity Seal (Sec.~\ref{se:unicity-certificate}). This is a wall-clock timestamp with one-second precision, set and verified by the BFT Core at the time the round is committed.
