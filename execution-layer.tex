...
To return value, the holder burns the token (Sec.~\ref{sec:burn-transaction}) to $\predi_\mathsf{burn}(H(\mathcal{R}))$, where the \emph{return reason} $\mathcal{R}$ commits to the external release: the recipient, a gross amount $v$ equal to the burned token's certified value for $\mathsf{aid}$, an optional relayer fee, a deadline, and the pinned configuration fields (Appendix~\ref{app:bridging-reason}). A fungible token might have been split (Sec.~\ref{sec:token-splitting}), causing fractional returns.
...
\item[Finality.] Each burned token verifies by $\Call{VerifyToken}{}$, with every certified transaction's leaf shown included under the shared root $h^*$; in particular the final state is a burn to $\predi_\mathsf{burn}(H(\mathcal{R}))$.
