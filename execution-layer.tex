...
A \emph{certified mint transaction} is a tuple $(T_0, \tau_{Q,0}, \tau_0, \pi_0)$, where $T_0 = (\sthash, D_\mathsf{mint})$ and
\[
...
\begin{itemize}
	...
	\end{itemize}
	\item $\tau_{Q,0}$ -- exclusive timeout of the mint certification request
...
\begin{enumerate}
	\item Choose request timeout $\tau_{Q,0}$
	\item $\txhash \gets H(D_\mathsf{mint}, \tau_{Q,0})$
	\item $h_T \gets H(\sthash, \txhash)$
	\item $\sigma \gets \mathsf{Sign}(\mathsf{sk}_\mathsf{mint}, h_T)$
	\item $Q \gets (\mathsf{pk}_\mathsf{mint}, \sthash, \txhash, \tau_{Q,0}, \sigma)$
	\item $\pi_0 \gets \unisrv(Q)$
\end{enumerate}

\paragraph{Verifying a certified mint transaction} $(T_0, \tau_{Q,0}, \tau_0, \pi_0)$ against trust base $\mathcal{T}$, using known, fixed mint keys:
\begin{enumerate}
	...
	\item require $\tau_0 < \tau_{Q,0}$
	\item $\txhash \gets H(D_\mathsf{mint}, \tau_{Q,0})$;\; require $H(\txhash, \tau_0) = \pi_0.v$
...
A \emph{certified transfer transaction} is a tuple $(T, u, \tau_Q, \tau, \pi)$, where $T = (\sthash, D)$:
\begin{itemize}
	...
	\item $\tau_Q$ -- exclusive timeout of the certification request
...
\begin{enumerate}
	\item Choose request timeout $\tau_Q$
	\item $\txhash \gets H(D, \tau_Q)$
	\item $h_T \gets H(\sthash, \txhash)$
	\item Create $u$ such that $\predi(\tau_\mathsf{exp}, h_T, u) = 1$ for an expected system time $\tau_\mathsf{exp}$
	\item $Q \gets (\predi, \sthash, \txhash, \tau_Q, u)$
	\item $\pi \gets \unisrv(Q)$
\end{enumerate}

\paragraph{Verifying a certified transfer transaction} $(T, u, \tau_Q, \tau, \pi)$ in state $(\predi, h)$:
\begin{enumerate}
	...
	\item require $\tau < \tau_Q$ \quad (request was validated before its timeout)
	\item $\txhash \gets H(T.D, \tau_Q)$;\; require $H(\txhash, \tau) = \pi.v$ \quad (transaction, timeout and reference time bind to the certified SMT leaf value)
...
\textbf{Computation}:
\begin{algorithmic}[0]
	\State \Parse $T_0^\mathsf{cert}$ \textbf{as} $(T_0, \tau_{Q,0}, \tau_0, \pi_0)$
	\If {$T_0.D_\mathsf{mint}.\alpha \ne \mathcal{T}.\alpha$} \Return \FALSE \EndIf
	\If {$\neg \Call{VerifyMintTransaction}{T_0, \tau_{Q,0}, \tau_0, \pi_0, \mathcal{T}}$} \Return \FALSE \EndIf
	...
	\For {$i \gets 1 \ldots n$}
		\State \Parse $T_i^\mathsf{cert}$ \textbf{as} $(T_i, u_i, \tau_{Q,i}, \tau_i, \pi_i)$
		\If {$\neg \Call{VerifyTransferTransaction}{T_i, u_i, \tau_{Q,i}, \tau_i, \pi_i, \predi^{i-1}, h^{i-1}, \mathcal{T}}$} \Return \FALSE \EndIf
...
A certification request from the execution layer is a tuple $Q = (\predi, \sthash, \txhash, \tau_Q, u)$, where:
\begin{itemize}
	\item $\predi$ -- spending predicate of the current owner
	\item $\sthash$ -- current state hash
	\item $\txhash$ -- transaction hash, committing to transaction data and $\tau_Q$
	\item $\tau_Q$ -- exclusive request timeout
...
\begin{enumerate}
	...
	\item Check timeout: verify $\tau < \tau_Q$
...
\begin{figure}[!htb]
    \begin{center}
        \begin{sequencediagram}
            ...
            \begin{messcall}{S}{$\mathcal{L} \cup (T, u, \tau_Q, \tau, \pinc)$}{R}
                \postlevel
                \postlevel
                \postlevel
                \postlevel
                \begin{call}{R}{\shortstack[l]{
                    ...
                    $\tau < \tau_Q$ \\
                    $\txhash \gets H(D,\tau_Q);\;\; H(\txhash,\tau) \stackrel{?}{=} \pinc.v$ \\
                    $\predi(\tau, H(\sthash, \txhash), u) \stackrel{?}{=} 1$ \\
                    $\mathcal{V}(H(\predi, T.\sthash), H(\txhash,\tau), \pinc, \mathcal{CD}_\beta, \mathcal{T}) \stackrel{?}{=} 1$ \\
                    Accept if all checks pass
                    }}{R}{}
                \end{call}
            \end{messcall}
        \end{sequencediagram}
        \caption{One Unicity transaction. The predicate $\predi$ represents the current ownership condition, and the capability to unlock $\predi'$ defines the new owner. The request is valid only for a round whose reference time $\tau$ satisfies $\tau<\tau_Q$.}\label{fi:exec-transaction-flow}
    \end{center}
\end{figure}
