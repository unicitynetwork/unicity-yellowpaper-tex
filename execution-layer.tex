...
Let $\mathbb{T}_\bot=\mathbb{T}\cup\{\bot\}$. Every transaction $T$ carries an exclusive request timeout $\tau_Q(T)\in\mathbb{T}_\bot$, where $\tau_Q(T)=\bot$ delegates the choice of deadline to the Unicity Service and requires no client clock. The timeout occupies a fixed position in the transaction encoding and is committed by the transaction hash in either case (Appendix~\ref{app:encodings}). A certified transaction carries its transaction, the reference time $\tau$ at which its leaf was created, and its inclusion proof. The concrete proof representation also carries the certification data needed to reconstruct the service request and the same reference time. Both must match. (Appendix~\ref{app:execution-layer}).

\subsubsection{Mint Transaction}\label{sec:mint-tx}

\index{mint transaction!structure}
A \emph{certified mint transaction} is a tuple $(T_0, \tau_0, \pi_0)$, where $T_0$ consists of the source state hash, the common data
\[
...
\begin{itemize}
	...
	\end{itemize}
	\item $\tau_Q(T_0)$ -- optional explicit timeout of the mint certification request
	\item $\tau_0\in\mathbb T$ -- reference time of the round in which the mint leaf was created
...
\paragraph{Certifying a mint transaction} $T_0$:
\begin{enumerate}
	\item Choose $\tau_Q(T_0)\in\mathbb{T}_\bot$; choosing $\bot$ delegates timeout assignment to the service and requires no local clock
	\item $\txhash \gets H(D_\mathsf{mint}, \tau_Q(T_0))$
	\item $h_T \gets H(\sthash, \txhash)$
	\item $\sigma \gets \mathsf{Sign}(\mathsf{sk}_\mathsf{mint}, h_T)$
	\item $Q \gets (\mathsf{pk}_\mathsf{mint}, \sthash, \txhash, \tau_Q(T_0), \sigma)$
...
\begin{enumerate}
	...
	\item require $\tau_0=\tau(\pi_0)$; derive $\txhash=H(D_\mathsf{mint},\tau_Q(T_0))$ and $v=H(\txhash,\tau_0)$
	\item if $\tau_Q(T_0)\ne\bot$, require $\tau_0 < \tau_Q(T_0)$
	\item require $v = \pi_0.v$ and $\mathcal{V}(H(\mathsf{pk}_\mathsf{mint}, \sthash), v, \pi_0, \mathcal{CD}_\beta, \mathcal{T}) = 1$
...
A \emph{certified transfer transaction} is a tuple $(T, \tau, \pi)$, where $T$ consists of the source state hash, the common transaction data $D$, and its request timeout:
\begin{itemize}
	...
	\end{itemize}
	\item $\tau_Q(T)\in\mathbb{T}_\bot$ -- optional explicit timeout of the certification request
	\item $\tau\in\mathbb T$ -- reference time of the round in which the leaf was created
	\item $\pi$ -- inclusion proof and associated certification data, including the unlocking argument $u$
\end{itemize}

\paragraph{Certifying a transfer transaction} $T$ in the current state $(\predi, h)$:
\begin{enumerate}
	\item Choose $\tau_Q(T)\in\mathbb{T}_\bot$; choosing $\bot$ delegates timeout assignment to the service and requires no local clock
	\item $\txhash \gets H(D, \tau_Q(T))$
	\item $h_T \gets H(\sthash, \txhash)$
	\item Create $u$ such that $\predi(\tau_\mathsf{exp}, h_T, u) = 1$ for an expected system time $\tau_\mathsf{exp}$
	\item $Q \gets (\predi, \sthash, \txhash, \tau_Q(T), u)$
	\item $\pi \gets \unisrv(Q)$
\end{enumerate}

\paragraph{Verifying a certified transfer transaction} $(T, \tau, \pi)$ in state $(\predi, h)$:
\begin{enumerate}
	...
	\item require $\tau=\tau(\pi)$; derive $\txhash=H(T.D,\tau_Q(T))$ and $v=H(\txhash,\tau)$
	\item if $\tau_Q(T)\ne\bot$, require $\tau < \tau_Q(T)$
	\item require $\predi(\tau, H(\sthash, \txhash), u) = 1$, where $u$ is recovered from the certification data accompanying $\pi$
	\item $\mathcal{V}(H(\predi, T.\sthash), v, \pi, \mathcal{CD}_\beta, \mathcal{T}) = 1$ \quad (inclusion proof valid and certified by the correct shard)
...
\textbf{Computation}:
\begin{algorithmic}[0]
	\State \Parse $T_0^\mathsf{cert}$ \textbf{as} $(T_0, \tau_0, \pi_0)$
	\If {$T_0.D_\mathsf{mint}.\alpha \ne \mathcal{T}.\alpha$} \Return \FALSE \EndIf
	\If {$\neg \Call{VerifyMintTransaction}{T_0, \tau_0, \pi_0, \mathcal{T}}$} \Return \FALSE \EndIf
	...
	\For {$i \gets 1 \ldots n$}
		\State \Parse $T_i^\mathsf{cert}$ \textbf{as} $(T_i, \tau_i, \pi_i)$
		\If {$\neg \Call{VerifyTransferTransaction}{T_i, \tau_i, \pi_i, \predi^{i-1}, h^{i-1}, \mathcal{T}}$} \Return \FALSE \EndIf
...
\begin{itemize}
	...
	\begin{itemize}
		\item $\tau \in \mathbb{T}$ -- certified request-validation reference time (Sec.~\ref{sec:time-extraction})
		\item $m \in \hashtype$ -- message hash $H(\sthash, \txhash)$, where $\txhash$ commits to the request timeout
...
For every request, $\tau$ is committed by the leaf value $H(\txhash, \tau)$ (Sec.~\ref{sec:request-validation}) and is carried with the transition. A verifier recomputes that value from the transaction data and the carried $\tau$ and requires it to equal the certified leaf value; a mismatch rejects. This makes re-validation deterministic and independent of which inclusion proof establishes the leaf. The invariant applies equally when the request timeout is omitted and assigned by the service.

Reading the time only from $UC.\mathcal{IR}.t$ identifies the validation time for a proof issued in the transition's own round. The SMT is append-only, so a leaf can be certified afresh against any later root, whose Input Record carries another round's reference time. The carried value $\tau(\pi)$ is therefore a property of the certified leaf rather than of the replaceable certificate chain.
...
A certification request from the execution layer is a tuple $Q = (\predi, \sthash, \txhash, \bar\tau_Q, u)$, where:
\begin{itemize}
	\item $\predi$ -- spending predicate of the current owner
	\item $\sthash$ -- current state hash
	\item $\txhash$ -- transaction hash, committing to transaction data and the timeout when one is explicit
	\item $\bar\tau_Q\in\mathbb{T}_\bot$ -- optional explicit, exclusive request timeout
...
\begin{enumerate}
	...
	\item Assign or recover the effective timeout $\widehat\tau_Q$ and verify $\tau < \widehat\tau_Q$
	\item Evaluate predicate: verify $\predi(\tau, m, u) = 1$
	\item Check uniqueness: verify $R[\mathsf{sid}] = \bot$
	\item If all checks pass: $R[\mathsf{sid}] \gets \lambda(Q,\tau)$
\end{enumerate}

For every request, the leaf value binds the reference time under which the request was validated. The BFT Core derives all leaf values from the declared transaction hashes and $\mathsf{CR}.\mathcal{IR}.t$ before verifying the consistency proof (Sec.~\ref{sec:derived-leaf-values}). Timeout omission affects transaction construction and admission policy, but not this commitment.

If the request is accepted, the Unicity Service returns an inclusion proof $\pinc$. If the request is expired, the state has already been spent, or the predicate evaluation fails, the request is rejected.
    ...
    \begin{center}
        \begin{sequencediagram}
            ...
            \begin{call}{S}{\shortstack[l]{
                    ...
                    $D = (\predi', x, \auxd')$ \\
                    $T = (\sthash, D)$         \\
                    choose $\bar\tau_Q\in\mathbb{T}_\bot$ \\
                    $\txhash \gets H(D,\bar\tau_Q)$ \\
                    $h_T = H(\sthash, \txhash)$ \\
            ...
            \begin{messcall}{S}{$\mathcal{L} \cup (T, \tau, \pinc)$}{R}
                \postlevel
                \postlevel
                \postlevel
                \postlevel
                \begin{call}{R}{\shortstack[l]{
                    ...
                    $v \stackrel{?}{=} \pinc.v$ \\
                    $\predi(\tau, H(\sthash, \txhash), u) \stackrel{?}{=} 1$ \\
                    $\mathcal{V}(H(\predi, T.\sthash), v, \pinc, \mathcal{CD}_\beta, \mathcal{T}) \stackrel{?}{=} 1$ \\
                    Accept if all checks pass
                    }}{R}{}
                \end{call}
            \end{messcall}
        \end{sequencediagram}
        \caption{One Unicity transaction. The predicate $\predi$ represents the current ownership condition, and the capability to unlock $\predi'$ defines the new owner. The request is valid only for a round whose reference time $\tau$ satisfies $\tau<\widehat\tau_Q$.}\label{fi:exec-transaction-flow}
    \end{center}
\end{figure}
