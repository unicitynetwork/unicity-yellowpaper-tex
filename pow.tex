...
\begin{itemize}[nosep]
  \item $H(\mathsf{tokenId}_\mathsf{reward}) \in \idtype$ --- the hash of the reward token identifier chosen by the miner. The actual token identifier remains in miner's sole custody and is revealed only when the token is redeemed (minted).
