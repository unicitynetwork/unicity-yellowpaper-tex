...
\begin{enumerate}[nosep]
  ...
  \item Prepares the payload commitments (Sec.~\ref{sec:pow-payload}):
    \begin{enumerate}[nosep]
      \item Chooses a reward token identifier $\mathsf{tokenId}_\mathsf{reward}$ unique within the network identifier $\alpha$ and stores it (e.g., by using a pseudo-random seed and a counter).
...
\begin{algorithm}[!htbp]
  ...
  \begin{algorithmic}[0]
    \Function{MintRewardToken}{$\alpha, B_h, \mathsf{sk}_\mathsf{miner}, \mathcal{T}$}
      ...
      \State $D_\mathsf{mint} \gets (\alpha, \predi', \mathsf{id}, \mathsf{ALPHA}, \mathsf{reason}, \auxd')$
      \State
