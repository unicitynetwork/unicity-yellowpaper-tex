...
\begin{enumerate}[nosep]
  ...
  \item Prepares the payload commitments (Sec.~\ref{sec:pow-payload}):
    \begin{enumerate}[nosep]
      \item Samples a 32-byte salt $\mathsf{salt}_\mathsf{reward} \overset{\$}{\leftarrow} \{0,1\}^{256}$ (min-entropy $\ge 128$ bits) and stores it as a secret. The salt is a commit-reveal preimage and MUST remain unpredictable to all other parties until revealed at minting time (Sec.~\ref{sec:pow-reward-minting}); deterministic derivations from publicly observable block data is NOT acceptable. The derived reward token identifier is $\mathsf{tokenId}_\mathsf{reward} = H(\mathsf{salt}_\mathsf{reward}, \alpha)$, globally unique across networks under collision-resistance of $H$.
