...
\begin{table}[!htbp]
  \begin{center}
    ...
    \begin{tabular}{|p{0.5cm}|p{3.5cm}|p{1.5cm}|p{7.5cm}|}\hline
      ...
      1. & $H(\mathsf{salt}_\mathsf{reward})$ & 32 B & Reward commitment leaf: hash of the reward salt, a 32-byte uniformly random value chosen by the miner (min-entropy $\ge 128$ bits). The preimage $\mathsf{salt}_\mathsf{reward}$ remains in the miner's custody and is revealed only at minting time as the $\mathsf{salt}$ field of the mint transaction, from which the token identifier $\mathsf{id} = H(\mathsf{salt}_\mathsf{reward}, \alpha)$ is derived (Sec.~\ref{sec:mint-tx}). \\\hline
...
\begin{enumerate}[nosep]
  ...
  \item Prepares the payload commitments (Sec.~\ref{sec:pow-payload}):
    \begin{enumerate}[nosep]
      \item Samples a 32-byte salt $\mathsf{salt}_\mathsf{reward} \overset{\$}{\leftarrow} \{0,1\}^{256}$ (min-entropy $\ge 128$ bits) and stores it. The derived reward token identifier is $\mathsf{id}_\mathsf{reward} = H(\mathsf{salt}_\mathsf{reward}, \alpha)$, globally unique across networks under collision-resistance of $H$.
