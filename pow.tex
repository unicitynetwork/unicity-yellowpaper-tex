...
\begin{table}[!htbp]
  \begin{center}
    ...
    \begin{tabular}{|p{0.5cm}|p{3.5cm}|p{1.5cm}|p{7.5cm}|}\hline
      ...
      1. & $H(\mathsf{tokenId}_\mathsf{reward})$ & 32 B & Reward commitment leaf: hash of the reward token identifier generated by the miner. The salt from which the token identifier was generated acts as a commit-reveal preimage: knowledge of it is what entitles a party to mint the corresponding reward token, hence the salt MUST be unpredictable to all other parties until the miner reveals it at minting time as the $\mathsf{salt}$ field of the mint transaction (Sec.~\ref{sec:mint-tx}). \\\hline
...
\begin{enumerate}[nosep]
  ...
  \item Prepares the payload commitments (Sec.~\ref{sec:pow-payload}):
    \begin{enumerate}[nosep]
      \item Samples a 32-byte salt $\mathsf{salt}_\mathsf{reward} \overset{\$}{\leftarrow} \{0,1\}^{256}$ (min-entropy $\ge 128$ bits) and stores it as a secret. The salt is a commit-reveal preimage and MUST remain unpredictable to all other parties until revealed at minting time (Sec.~\ref{sec:pow-reward-minting}); deterministic derivations from publicly observable block data are NOT acceptable. The derived reward token identifier is $\mathsf{tokenId}_\mathsf{reward} = H(\mathsf{salt}_\mathsf{reward}, \alpha)$, globally unique across networks under collision-resistance of $H$.
...
\begin{table}[!htbp]
  \begin{center}
    ...
    \begin{tabular}{|p{4cm}|p{9.5cm}|}\hline
      ...
      Token identifier salt ($\mathsf{salt}$) & The 32-byte reward token salt $\mathsf{salt}_\mathsf{reward}$ that yields the reward token identifier $\mathsf{tokenId}_\mathsf{reward} = H(\mathsf{salt}_\mathsf{reward}, \alpha)$ whose hash $\ell_0 = H(\mathsf{tokenId}_\mathsf{reward})$ was committed via \texttt{payloadRoot} of the mined block. \\\hline
...
\begin{algorithm}[!htbp]
  ...
  \begin{algorithmic}[0]
    \Function{MintRewardToken}{$\alpha, B_h, \mathsf{sk}_\mathsf{miner}, \mathcal{T}$}
      ...
      \State $u_0 \gets \mathsf{Sign}(\mathsf{sk}_\mathsf{mint}, H(\sthash, \txhash))$  \Comment{Known constants, network specific}
...
\begin{algorithm}[!htbp]
  ...
  \begin{algorithmic}[0]
    \Function{VerifyRewardMint}{$D_\mathsf{mint}, \mathcal{T}$}
      ...
      \State $\mathsf{tokenId}_\mathsf{reward} \gets H(D_\mathsf{mint}.\mathsf{salt}, D_\mathsf{mint}.\alpha)$ \Comment{Reward token identifier}
      \State ensure($\ell_0 = H(\mathsf{tokenId}_\mathsf{reward})$) \Comment{Token identifier matches the reward commitment}
      \State
