...
\[
  \texttt{payloadRoot} \;=\; H(H(\mathsf{salt}_\mathsf{reward}) \;\|\; H(\mathsf{cbor}(T_e))) \;.
\]
...
\begin{table}[!htbp]
  \begin{center}
    ...
    \begin{tabular}{|p{0.5cm}|p{3.5cm}|p{1.5cm}|p{7.5cm}|}\hline
      ...
      1. & $H(\mathsf{salt}_\mathsf{reward})$ & 32 B & Reward commitment leaf: hash of the reward salt $\mathsf{salt}_\mathsf{reward}$, a 32-byte secret sampled with at least $128$ bits of min-entropy and held in the miner's custody. The salt acts as a commit-reveal preimage: knowledge of $\mathsf{salt}_\mathsf{reward}$ is what entitles a party to mint the corresponding reward token, hence the salt MUST be unpredictable to all other parties until the miner reveals it at minting time as the $\mathsf{salt}$ field of the mint transaction. The derived token identifier is $\mathsf{id} = H(\mathsf{salt}_\mathsf{reward}, \alpha)$ (Sec.~\ref{sec:mint-tx}). \\\hline
...
\begin{enumerate}[nosep]
  ...
  \item Prepares the payload commitments (Sec.~\ref{sec:pow-payload}):
    \begin{enumerate}[nosep]
      \item Samples a 32-byte salt $\mathsf{salt}_\mathsf{reward} \overset{\$}{\leftarrow} \{0,1\}^{256}$ (min-entropy $\ge 128$ bits) and stores it as a secret. The salt is a commit-reveal preimage and MUST remain unpredictable to all other parties until revealed at minting time (Sec.~\ref{sec:pow-reward-minting}); deterministic derivations from publicly observable block data are NOT acceptable. The derived reward token identifier is $\mathsf{id}_\mathsf{reward} = H(\mathsf{salt}_\mathsf{reward}, \alpha)$, globally unique across networks under collision-resistance of $H$.
      ...
      \item Computes $\texttt{payloadRoot} \;=\; H(H(\mathsf{salt}_\mathsf{reward}) \;\|\; H(\mathsf{cbor}(T_e)))$.
    \end{enumerate}
...
The steps to validate a block are documented by Algorithm~\ref{alg:pow-validate}.
  ...
  \begin{algorithmic}[0]
    \Function{ValidatePoWBlock}{$B, \mathcal{T}_\mathsf{local}$}
      ...
      \If{$\mathsf{cbor\_Te} \ne \Call{CborNil}{}$}
        ...
        \State ensure($T_e.\alpha = T_{e-1}.\alpha = \mathcal{T}_\mathsf{local}.\alpha$) \Comment{Checkpoint is for the same network}
...
\begin{table}[!htbp]
  \begin{center}
    ...
    \begin{tabular}{|p{4cm}|p{9.5cm}|}\hline
      ...
      Network identifier ($\alpha$) & The Unicity network instance in which the reward token can be minted \\\hline
      Token type ($\mathsf{ty}$) & $\mathsf{ALPHA}$ \\\hline
      Owner predicate ($\predi'$) & Miner's chosen spending predicate \\\hline
      Token identifier salt ($\mathsf{salt}$) & $\mathsf{salt}_\mathsf{reward}$: the 32-byte salt whose hash $\ell_0 = H(\mathsf{salt}_\mathsf{reward})$ is committed via \texttt{payloadRoot} of the mined block. The derived token identifier is $\mathsf{id} = H(\mathsf{salt}_\mathsf{reward}, \alpha)$ \\\hline
      Mint reason ($\mathsf{reason}$) & Composite proof structure defined in Table~\ref{ta:pow-mint-reason} \\\hline
      Auxiliary payload ($\auxd'$) & Initial fungible payload containing value $\Call{RewardRate}{h}$ for $\mathsf{ALPHA}$ at block height $h$ \\\hline
    \end{tabular}
  \end{center}
\end{table}
...
\begin{table}[!htbp]
  \begin{center}
    \caption{Mint Reason for Block Reward Token.}\label{ta:pow-mint-reason}
...
\begin{algorithm}[!htbp]
  ...
  \begin{algorithmic}[0]
    \Function{MintRewardToken}{$\alpha, B_h, \mathsf{sk}_\mathsf{miner}, \mathcal{T}$}
      ...
      \State
      \State \Comment{Step 3: Construct mint reason and auxiliary payload}
      \State $\mathsf{reason} \gets (B_h, \mathsf{blocks}, \mathsf{cert}_\mathsf{FG})$
      \State $\auxd' \gets \{(\mathsf{ALPHA}, \Call{RewardRate}{h})\}$
      \State
      \State \Comment{Step 4: Construct mint transaction (Sec.~\ref{sec:mint-tx})}
      \State $\mathsf{salt} \gets \mathsf{salt}_\mathsf{reward}$  \Comment{Preimage of $\ell_0 = H(\mathsf{salt}_\mathsf{reward})$ committed in $B_h.\texttt{payloadRoot}$}
      \State $\predi' \gets \Call{OwnerPredicate}{\mathsf{sk}_\mathsf{miner}}$
      \State $D_\mathsf{mint} \gets (\alpha, \predi', \mathsf{salt}, \mathsf{ALPHA}, \mathsf{reason}, \auxd')$
      \State
      \State \Comment{Step 5: Submit to Unicity Service}
      \State $\sthash \gets H(H(\mathsf{salt}, \alpha), \mathsf{MINT\_SUFFIX})$ \Comment{$\mathsf{id} = H(\mathsf{salt}, \alpha)$, then mint source state hash}
      \State $\txhash \gets H(D_\mathsf{mint})$
      \State $\sigma \gets \mathsf{Sign}(\mathsf{sk}_\mathsf{mint}, H(\sthash, \txhash))$  \Comment{Known constants, network specific}
      \State $Q \gets (\mathsf{pk}_\mathsf{mint}, \sthash, \txhash, \sigma)$
      \State \Call{SubmitToUnicityService}{$Q$}
    \EndFunction
  \end{algorithmic}
\end{algorithm}

Verification of a reward token by a recipient requires checking the mint reason and auxiliary payload:

\begin{algorithm}[!htbp]
  \caption{Block Reward Mint Reason Verification}\label{alg:pow-reward-verify}
  \begin{algorithmic}[0]
    \Function{VerifyRewardMint}{$D_\mathsf{mint}, \mathcal{T}$}
      \State $(B_h, \mathsf{blocks}, \mathsf{cert}_\mathsf{FG}) \gets D_\mathsf{mint}.\mathsf{reason}$
      \State
      ...
      \State ensure($\ell_0 = H(D_\mathsf{mint}.\mathsf{salt})$) \Comment{Reward commitment matches the salt revealed in the mint transaction}
      \State
      ...
      \State ensure($\mathsf{cert}_\mathsf{FG}.C^\mathsf{r}.\alpha = \mathcal{T}.\alpha$)
      ...
      \State ensure($D_\mathsf{mint}.\alpha = \mathcal{T}.\alpha$)
      \State ensure($D_\mathsf{mint}.\mathsf{ty} = \mathsf{ALPHA}$)
      \State ensure($\Call{ValueOf}{D_\mathsf{mint}.\auxd', \mathsf{ALPHA}} = \Call{RewardRate}{B_h.\mathsf{height}}$)
      \State \Return $1$
    \EndFunction
  \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[!htbp]
  ...
  \begin{algorithmic}[0]
    ...
    \Function{PrepareCheckpoint}{}
      ...
      \State $\mathcal{T}_\mathsf{new} \gets \Call{GetTrustBase}{\mathcal{T}_\mathsf{local}.\alpha, e_\mathsf{latest}+1}$
      ...
      \ForAll{$T_e \in \mathcal{T}_\mathsf{new}$}
        \State ensure($T_e.\alpha = \mathcal{T}_\mathsf{local}.\alpha$) \Comment{Trust Base entry belongs to this network}
...
The procedure is summarized in Algorithm~\ref{alg:maxi-verify}. It is a wrapper around the standard token verification algorithm of Sec.~\ref{sec:token-verification}: it derives the Unicity Trust Base $\mathcal{T}_\mathsf{PoW}$ from the live PoW chain, then calls $\mathsf{VerifyToken}$. The PoW validator may be operated by the verifier locally or hosted by a trusted entity; in either case it is accessed over the standard PoW node RPC interface.

\begin{algorithm}[!htbp]
  \caption{Maximalist Token Verification}\label{alg:maxi-verify}
  \begin{algorithmic}[0]
    \State \emph{State}:
    \State $\mathcal{N}_\mathsf{PoW}$: handle to a live, fully synced PoW validator
    \State $d_\mathsf{maxi}$: required confirmation depth
    \State
    \Function{VerifyTokenMaxi}{$\mathcal{L}$}
      \State \Comment{Input: token $\mathcal{L} = (T_0^\mathsf{cert}, T_1^\mathsf{cert}, \ldots, T_n^\mathsf{cert})$}
      \State
      \State \Comment{Step 1: Identify the latest UC referenced by the token}
      \State $\pi_n \gets$ inclusion proof component of $T_n^\mathsf{cert}$ \Comment{Last transaction in the chain}
      \State $UC^\star \gets \pi_n.UC$
      \State $\alpha \gets UC^\star.C^\mathsf{r}.\alpha$ \Comment{Network identifier}
      \State $e^\star \gets UC^\star.C^\mathsf{r}.e_r$ \Comment{BFT Core epoch in which $UC^\star$ was issued}
      \State
      \State \Comment{Step 2: Wait for epoch $e^\star$ closure to be checkpointed and PoW-finalized}
      \State $h^\star \gets \bot$;\quad $h_\mathsf{tip} \gets 0$
      \While{$h^\star = \bot \;\textbf{or}\; h_\mathsf{tip} - h^\star < d_\mathsf{maxi}$}
        \State \Call{WaitForNewBlocks}{$\mathcal{N}_\mathsf{PoW}$}
        \State $h_\mathsf{tip} \gets \mathcal{N}_\mathsf{PoW}.\Call{GetChainTip}{}.\mathsf{height}$
        \State \Comment{Lowest PoW height carrying a $T_e$ with $e \ge e^\star{+}1$, or $\bot$ if none}:
        \State $h^\star \gets \mathcal{N}_\mathsf{PoW}.\Call{HeightOfFirstCheckpoint}{e \ge e^\star + 1}$
      \EndWhile
      \State
      \State \Comment{Step 3: Pull the locally verified Trust Base from the PoW node}
      \State $\mathcal{T}_\mathsf{PoW} \gets \mathcal{N}_\mathsf{PoW}.\Call{GetVerifiedTrustBase}{\alpha}$
        \Comment{Entries reconstructed from PoW snapshots (Sec.~\ref{sec:snapshot-publishing})}
      \State ensure($\mathcal{T}_\mathsf{PoW}.\alpha = \alpha$)
      \State ensure($e^\star \le \max\{e : T_e \in \mathcal{T}_\mathsf{PoW}\} - 1$) \Comment{Token's epoch is closed in $\mathcal{T}_\mathsf{PoW}$}
      \State
      \State \Comment{Step 4: Standard token verification under the PoW-derived trust base}
      \State ensure($\Call{VerifyToken}{\mathcal{L}, \mathcal{T}_\mathsf{PoW}}$) \Comment{Algorithm of Sec.~\ref{sec:token-verification}}
      \State
      \State \Comment{Step 5: Validate aggregator node's trustless operation based on consistency proofs}
      \State ensure($\Call{VerifyConsistency}{\mathcal{L}, \mathcal{N}_\mathsf{PoW}, \mathcal{T}_\mathsf{PoW}}$)
      \State \Return $1$
    \EndFunction
  \end{algorithmic}
\end{algorithm}
...
Token verification is fast: a transaction is finalized and an inclusion proof returned within a few seconds of submission to the Unicity Service. The recipient runs the standard $\mathsf{VerifyToken}$ algorithm of Sec.~\ref{sec:token-verification} directly against the locally maintained $\mathcal{T}$, without consulting external data beyond the previously obtained Trust Base.
