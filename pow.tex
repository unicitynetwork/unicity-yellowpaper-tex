...
\[
  \texttt{payloadRoot} \;=\; H(H(\mathsf{id}_\mathsf{reward}) \;\|\; H(\mathsf{cbor}(T_e))) \;.
\]
...
\begin{table}[!htbp]
  \begin{center}
    ...
    \begin{tabular}{|p{0.5cm}|p{3.5cm}|p{1.5cm}|p{7.5cm}|}\hline
      ...
      1. & $H(\mathsf{id}_\mathsf{reward})$ & 32 B & Reward commitment leaf: hash of the reward token identifier generated by the miner. The salt from which the token identifier was generated acts as a commit-reveal preimage: knowledge of it is what entitles a party to mint the corresponding reward token, hence the salt MUST be unpredictable to all other parties until the miner reveals it at minting time as the $\mathsf{salt}$ field of the mint transaction (Sec.~\ref{sec:mint-tx}). \\\hline
...
\begin{enumerate}[nosep]
  ...
  \item Prepares the payload commitments (Sec.~\ref{sec:pow-payload}):
    \begin{enumerate}[nosep]
      ...
      \item Computes $\texttt{payloadRoot} \;=\; H(H(\mathsf{id}_\mathsf{reward}) \;\|\; H(\mathsf{cbor}(T_e)))$.
    \end{enumerate}
...
\begin{table}[!htbp]
  \begin{center}
    ...
    \begin{tabular}{|p{4cm}|p{9.5cm}|}\hline
      ...
      Token identifier salt ($\mathsf{salt}$) & The 32-byte reward token salt $\mathsf{salt}_\mathsf{reward}$ that yields the reward token identifier $\mathsf{id}_\mathsf{reward} = H(\mathsf{salt}_\mathsf{reward}, \alpha)$ whose hash $\ell_0 = H(\mathsf{id}_\mathsf{reward})$ was committed via \texttt{payloadRoot} of the mined block. \\\hline
...
\begin{algorithm}[!htbp]
  ...
  \begin{algorithmic}[0]
    \Function{MintRewardToken}{$\alpha, B_h, \mathsf{sk}_\mathsf{miner}, \mathcal{T}$}
      ...
      \State $\mathsf{salt} \gets \mathsf{salt}_\mathsf{reward}$  \Comment{such that $\ell_0 = H(H(\mathsf{salt}, \alpha))$ was committed via $B_h.\texttt{payloadRoot}$}
...
\begin{algorithm}[!htbp]
  ...
  \begin{algorithmic}[0]
    \Function{VerifyRewardMint}{$D_\mathsf{mint}, \mathcal{T}$}
      ...
      \State $\mathsf{id}_\mathsf{reward} \gets H(D_\mathsf{mint}.\mathsf{salt}, D_\mathsf{mint}.\alpha)$ \Comment{Reward token identifier}
      \State ensure($\ell_0 = H(\mathsf{id}_\mathsf{reward})$) \Comment{Token identifier matches the reward commitment}
      \State
