...
\section{Governance of the Dynamic Distributed Machine}\label{sec:governance-dynamic}
...
For the BFT Core committee, the normative mechanism is delegated Proof of Stake, specified in Sec.~\ref{sec:staking}: committee membership and voting power derive from bonded stake, the election is a deterministic function of the certified staking registry, and the Governance Body's role for the committee reduces to setting the economic parameters. The mechanisms below describe the modular framing and the bootstrap path.

\section{Proof of Authority}

In the Proof of Authority mechanism, a permissioned, \emph{authorized} entity provides the governance information by sending signed transactions to the governance partition. Proof of Authority is the bootstrap mechanism: the system launches under the support and control of the Unicity Foundation, and the on-chain DPoS processes take over along the decentralization roadmap.
...
Proof of Stake mechanism\index{PoS (Proof of Stake)} is based on the fact that there is a finite amount of certain backing asset, in Unicity's case it is the native currency, UCT. When each validator presents a proof of locking a certain amount of UCT, it is guaranteed, that there is a real capital cost involved and hard economical limits of potential power grab.

Also, the staking mechanism provides a commitment and shared ownership structure for the Unicity Community.

Validator's voting power in consensus correlates to its staked amount. That is, votes have weights, and the quorum size is defined as the arithmetic sum of stakes of agreeing validators which is necessary to reach a consensus. The normative stake-weighted quorum rule, the bonding and delegation mechanics, slashing and rewards are specified in Sec.~\ref{sec:staking}.

The leader election algorithm may use stake amounts as an input, so that the probability of becoming a leader correlates to the stake amount.
...
\begin{description}
	\item[Incentives] are payouts in UCT currency to incentivize validator participation in the Unicity System. There are incentives for staking (reward for capital cost) and incentives for providing secure and highly available validation service (reward for computation effort).
	...
	\item[Delegated Proof of Stake] is an extension to PoS where external parties can delegate their stake to specific validator nodes. Crucially, this democratizes the staking, so that non-sophisticated parties can participate, and also brings in external knowledge about the trustfulness of validators: delegators choose which validators they are placing their stakes on. This is the enshrined mechanism for the BFT Core committee (Sec.~\ref{sec:staking}).
\end{description}
...
The process produces change records (called Validator Assignment Records), which assign validators to specific partitions and shards, as shown in Table~\ref{ta:var}. Its scope is the aggregation layer: for the BFT Core committee, the assignment record is not produced by a Governance Process but derived deterministically from the certified staking registry by the election function (Sec.~\ref{sec:election}).
...
The orchestration of shard split is executed as follows:
\begin{enumerate}
	...
	\item At the epoch switch, the validators joining $\sigma\|0$ and $\sigma\|1$ obtain the last certified state of $\sigma$. Majority of the validators after split should be pre-existing, these are already pre-synced and known to be able to execute.
